...
| 2c | Data API exposure proof (review of 871933a, corrected per reviews of be433e0 and 3e7d898) | **Prepared, blocked on gate 2 for execution**: `api_denial.sh` corrected per review — real-UUID identity parsing (hard exit 2), fixture-based storage tests, transport-error separation, fail-closed storage-list handling (5xx/unexpected statuses FAIL; 200 must be a valid list; disclosure is a hard failure at any status), cleanup done via a real conditional with UUID-validated session id + removal regression check (warnings, never silent). Failure semantics are now executable-tested: `tests/run_selftest.sh` + `tests/selftest/mock_api.py` run 10 scenarios against a local mock (missing config → 2, invalid auth response → 2, invalid UUID → 2, unreachable host → 2, missing fixture 404 → 2, owner-unreadable existing fixture 403 → 2 (distinct scenario), 5xx list → FAIL, disclosure → HARD FAIL, healthy → 0 with no warnings, cleanup 500 → WARN + exit 0; credential-leak grep in every scenario; count is emitted from a runtime counter, not hardcoded). Self-test wired into the CI shell-lint job | Dev B + D |
