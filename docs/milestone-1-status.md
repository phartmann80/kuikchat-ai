...
| 2c | Data API exposure proof (review of 871933a) | **Prepared, blocked on gate 2 for execution**: `supabase-personal/tests/api_denial.sh` — HTTP-level negative tests for anon AND authenticated (helper RPC 404s, `Accept-Profile`/`Content-Profile: private` 406s, restricted public RPC denial, empty unauthenticated table reads, private storage denial). Exposed-schema configuration documented in supabase-personal/README, ADR-001, smoke-test plan. New release gate recorded: `private schema is not exposed through the Personal Data API` | Dev B + D |
